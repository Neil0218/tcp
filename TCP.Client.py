...
def send_initialization_request(client_socket, n_blocks):
...
def client_program(file_path, server_ip, server_port, min_len, max_len):
    ...
    if text_length < min_len or min_len >= max_len:
        print("文本长度不符合要求。")
        return
    ...
    blocks = []
    while text:
        block_len = min(random.randint(min_len, max_len), len(text))
        blocks.append(text[:block_len])
        text = text[block_len:]

    send_initialization_request(client_socket, len(blocks))
...
if __name__ == "__main__":
    ...
    client_program("tcp.txt", server_ip, int(server_port), 6, 10)
